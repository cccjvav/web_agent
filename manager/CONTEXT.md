...
## 当前状态
- **F83（2026-09-26，本会话）：** 用户对D4逐项确认：外部MCP迁入设置标签页，多模型博弈留在网页工作台（分支/合并/总结在网页Chat，插件Chat只显示共识事件；将来另立“插件Chat分支与总结”）。R6第二期第4批：转发层放行外部MCP登记四个接口（external/request、consensus仍拒绝），operations的公网登记与stdio启动改用VS Code确认框，stdio启动改为弹框前置忙（异步框下连点会弹两次）；登记字段在标签页显示，导航名改为“工具接入与审批”。单元18、浏览器5种变异全红；自查更正一处文档（stdio只得到主机的PATH等少数变量）。原第4批手册顺延为第5批。详见[第83组](stages/s10-upstream-adoption.md#第83组r6第二期第4批外部mcp迁入设置标签页2026-09-26)。
