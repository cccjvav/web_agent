...
## 用户环境与最终验收
- 用户指定：剩余施工完成后，从Arena实际连接本机WebAgent MCP，选择本项目根；先核对工具/ping/workspace_info的身份/root、Git提交与现有改动，再受控测试。R8结果：[R8实机验收记录](../review/R8实机验收记录-2026-09-24.md)总判定通过（M1–M4覆盖，M5/9.2、7.8双凭据、浏览器回归未执行）；沙箱不能代签。用户下一步选定：插件一体化启动返工，[方案稿](../docs/development/插件一体化启动返工方案.md)待用户确认D1–D4，未施工；用户澄清网页工作台保留，只是插件版不应依赖它。本轮逐步手册：[R8本轮Arena实机验收](../docs/guides/R8本轮Arena实机验收.md)（用户环境：Windows桌面VS Code＋集成CMD＋Conda＋系统Node，仓库根工作区，Quick Tunnel）。
