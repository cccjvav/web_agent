...
## 当前状态
- **本轮F60：** 已先复审80de651及相邻启动/IPC/打包合同，再改异步npm准备/await/停止接线；保留120/180秒工作预算，直接子进程另观察至多1秒，未知非零。真实取消先红后绿，本地97/97；源码ce68560 / CI35662916656精确九job成功；验证与范围见阶段10第60组、独立报告第12节。
- **边界仍开放：** 本组只修原同步命令链的在途取消/期限，不保证npm后代、部分安装回滚或同步磁盘步骤可抢占。下一轮先复核本组，再处理旧后端依赖fallback异步npm install尚无独立deadline的问题。
