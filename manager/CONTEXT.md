...
## 当前状态
- **F71（本会话`arena/01a0d084-web-agent`，起点4a868ae=01a0ce8d的F70末提交）：** 用户确认顺序：①修无会话调用者合并 ②用户用Arena实机验收第七批与本批（R8）③新规范2026-07-28按需做成新旧两代并存。①已修：无会话远程调用的callerKey由`名称@req.ip`改为追加由已验证凭据派生的盐化HMAC标签（session.sessionKey），经隧道同为127.0.0.1的URL密钥与各OAuth客户端不再互读命令输出/get_logs；同一凭据的多个对话仍合并（须initialize区分）。新增mcpCallerIsolation基线红，本地110/110。详见[阶段10第71组](stages/s10-upstream-adoption.md#第71组接手01a0ce8d无会话调用者按凭据区分2026-09-24会话01a0d084)。
