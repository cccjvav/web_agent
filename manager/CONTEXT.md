...
## 当前状态
- **随时交接入口已补核：** 先读本页→agents→[阶段10即时接手检查](stages/s10-upstream-adoption.md#即时接手检查2026-09-21)。上轮完整基线dcc321b / CI35614829672九项、本地94/94；本轮变更以Git HEAD和精确SHA的CI为准，不借基线代签。
- **本轮继续R4可观察性修复：** 已红测旧annotation漏掉生命周期中段，新增有限白名单摘要，保留原期限/失败语义；验证见阶段10。根因未关闭，下一步优先分析新失败阶段，非重复跑到绿。
- **R5第六包：Windows开始菜单本机回收快捷入口已接入。** 固定PS→本机Node→既有交互CLI，双TTY/零参数、同会话重复点击互斥；不解析工作区/装依赖/启动Bridge，不新增网页/MCP权限。仍须RECYCLE；a22428a CI8/9，新入口/安装器通过，但Windows20旧MCP echo约30秒超时，根因未定。面板/桌面未验与探针暂停保留。
- **R5第五包：收据/身份辅助脱敏诊断已接入。** 延用默认关闭的调试开关，记录有限阶段/耗时/白名单退出信息，不带收据/路径或原输出，不改8秒期限/重试/权限；6fd13b6 CI35611958857九项通过，本地debug关/开93/93，但历史根因未复现/未关闭。037b0ce的CI35603908774为8/9，主机矩阵通过、暂停Probe打包失败未接手；前次Windows22根因仍待。
- **R5第四包：真实Windows进程DACL拒绝回归已验证。** 仅测试专用宿主/子进程ACL，目标不可访问与活宿主查询拒绝都必须不终止；不改产品入口或权限；e6e9aac真实Windows矩阵通过，本地92/92，首轮夹具失败保留；后续befee7d Windows22原收据链失败（8/9），根因未定，私有线程令牌修订8d49bea的CI35603422500九项通过，但先前失败根因仍未关闭。图形/跨用户/实际PID复用仍待，探针暂停不变。
- **R5第三包：Windows本机交互预览/确认与稳定句柄回收已接入并经CI验证。** 仅新保护记录、标准隧道根进程、父PID/创建时间/映像匹配且宿主已死；活实例、未知及旧记录跳过。无图形按钮/API；ffb6f72真实Windows矩阵已过，目标/宿主DACL拒绝已由第四包补验，跨用户与用户桌面仍待，详见阶段10。
- **R5第二包历史：Windows收据完整性保护，单独不授予清理权。** 新记录CurrentUser DPAPI封装，失败不降级明文，旧v1保持unverified；保护标签不证明WebAgent来源或授予终止权限。本地89/89，实现721686c的CI35596632247九项通过；本轮第三包接Windows句柄和终端入口，边界见阶段10。
- **R5用户已授权安全残留回收，首包先做归属与只读检测。** 三个隧道启动点记私有无Token收据，CLI有限检测并保护活实例；只读报告所有canCleanup=false，图形按钮/API仍未提供。旧版无记录/不确定身份不认领；后续第三包已接稳定句柄与本机确认，不复用停止Bridge。本地88/88，修订9b8ed65的CI35590001850九项通过；首轮Windows夹具失败及修订证据见阶段10。
- **R2/R3继续：外部拒绝响应仍提前保存SID的相邻缺陷已红测并修复。** HTTP/SSE候选SID延后至RPC接受后提交；真实后续请求验证旧SID保留，未知结果不重放，有效结果/通知兼容保留。实现19eaa16的CI35587380854精确SHA九项通过、本地87/87；边界见阶段10，其他工作包不据此关闭。
- **R2/R3继续：外部MCP矛盾响应误判成功已修复。** HTTP JSON/SSE与stdio禁止result和error:null/false/0并存；登记失败清理，批准后未知结果不重放。其余R2–R9及文档逐句/实机仍开放，探针继续暂停；实现d520ae7的CI35586118614精确SHA九项通过、本地87/87，范围见阶段10。
- **R4历史Windows22超时追查：根因仍未定位，已补诊断首包。** 重新取到35125290301的annotations，确认echo在30519ms超时且无输出、worker在10秒startup超时；完整日志仍EOF。对照历史代码后只加默认关闭的阶段元数据、Windows CI启用及运行器结果元数据，不加时限/重试、不改进程保护。实现144833a的CI35560613257九项通过，Linux debug开/关分别87/87；R4仍未结案。
- **第七批原生requestJson响应边界已交付，本地86/86及真实Chromium通过，实现c37ef28的CI35559183087九项成功。** 8MiB字节上限、15秒总时限、断流清理与拒绝重定向；不改变业务JSON判断、审批/PTY授权或unknown不重放。真实HTTP覆盖轮换/停止消费，实际Windows仍未验，探针继续暂停。
- **第六批窄屏/页签ARIA修复已交付，本地85/85及真实Chromium/两条axe规则通过，实现e32974a的CI35545037493九项成功。** 从干净4d6c580接续，先真实浏览器红测主区零宽；改为窄屏显式单工作面、tablist外关闭当前页，保留草稿/dirty确认/焦点和桌面双栏。探针暂停不变，实际Windows/读屏器仍未验。
- **第五批原生聊天可靠终态实现已推送，931d4e9的CI35542931972九项成功，本地85/85与真实Chromium通过；证据提交2111f0e的CI35543099280为8/9，Windows24依赖审计失败原因未明，不能据前次绿灯关闭。** 先红测302被误判成功，再修NDJSON失败传播/预算/终态、两种原生消费者与失败历史；不改审批/文件/MCP权限。后续第六批补窄屏/ARIA，第七批现补requestJson响应边界；实际IDE及其他未验项保留，探针继续暂停。
...
- **R2/R3未完成清单（当前最近施工先看上方R4/R5）**：新文件patch的hash/块处理；RPC准入/批内ID与busy会话驱逐；资源caller/目录ACL；原生聊天可靠终态；窄屏/ARIA。busy会话/会话头已交付；第二批补RPC envelope/协商版本/批内ID与预算预检，第三批`95c3330`新增缺失目标hash/新建块校验，[CI35534909704](https://github.com/cccjvav/web_agent/actions/runs/35534909704)九项成功；第四批补可信资源caller/目录ACL与错误hash指引；第五批修原生流确认与失败历史；第六批现修窄屏/页签ARIA，其他UI/实机项仍待。正式回归须先锁正确合同再最小修复，不用整体替换掩盖问题。
...
- 首轮390/320px中心区宽度0、欢迎页遮挡及tablist子角色错误，第六批已补对应回归和局部修复；旧浏览器绿灯确未覆盖这些缺陷。字体优化是建议，Linux审查字体回退不是用户Windows效果认证。
