...
## 当前阶段
- **阶段10：继续上游借鉴与文档整顿**。Tasks隔离/刷新恢复与初步编辑预览（核心0.7.2）已交付；所有者权限与Chat/Bridge互斥已交付，经典单文件受保护回退已交付，显式公网HTTPS出站已交付，当前推进跨文件/原生预览回退等剩余范围，并继续文档逐句审查与旧指南退役、过期内容清理与实际缺口复核；见[阶段10](stages/s10-upstream-adoption.md)。阶段9工作区调整已交付。
...
- 当前产品代码7790e55bfbaf481ce52a1942e9e72da56e6b7f81（公网HTTPS、Node18取消修正与旧审计归档），[CI35105944367](https://github.com/cccjvav/web_agent/actions/runs/35105944367)九项成功，含真实Chromium、Windows安装器和七组Node矩阵。本地79测试文件通过，240源码/28目录/109排除。首轮b321432为7/9通过，失败原因与修正见阶段10；隔离TLS不是实际公网服务验收。核心0.7.2、Companion0.5.2未改；新控件真实Windows/手机人工验收仍待。
- 持久登录仍延期；Chat API确切后端身份是用户约定的后续任务。启发式结果提供参考，不承诺绝对鉴定。

- 文档审查与施工继续：[逐句修订与剩余施工](../review/SEMANTIC_REVIEW_2026-09-16.md)。已修Plan无效模型回退、直接工具API成功外包及MCP在途取消；又修密钥轮换保存失败仍报成功，协议/存储/运行配置详解累计全文复核4篇，其余文档仍待审。本批已实现四分类所有者控制、主机模式互斥及后台/审批屏障，本地78测试文件通过，237源码/28目录/109排除；本批CI35097629168九项已确认成功，不继承旧绿灯。不是仅讨论缺口。手机固定Bridge，同类型并发保留。新增控件不继承旧手机验收。当前批测试/CI以阶段10记录为准，未沿用旧绿灯。
