...
- 当前产品代码6ef3ac7cb2a618a1e773b9a5a2022ed46b121daf（跨文件内容检查点），[CI35112168535](https://github.com/cccjvav/web_agent/actions/runs/35112168535)九项成功：七组Node矩阵、安装器及Chromium真实双文件建立/预览/取消/恢复。81测试文件通过，244源码/28目录/110排除。原生窗口diff/撤销仍待实机；新检查点不继承旧手机验收。核心0.7.2、Companion0.5.2未改。
- 持久登录仍延期；Chat API确切后端身份是用户约定的后续任务。启发式结果提供参考，不承诺绝对鉴定。

- 当前续作：三种恢复入口（经典保存/原生草稿/跨文件内容检查点）均有明确范围与验证。全文重写架构导读/组件说明，清除计算器默认、无安装包、无Playwright、并行Chat/Bridge及许可证泛化等旧结论，四层教学和目录职责导航保留。全项目/新建删除/外部副作用回滚不在检查点能力内，全仓逐句审查仍未完。
