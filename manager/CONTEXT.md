...
- 最近完整通过的实现基线28f26e166be5294bc2963b30e195e800ef48f05d（第28组第三方候选跨端修复），[CI35263924819](https://github.com/cccjvav/web_agent/actions/runs/35263924819)九项逐项成功；本地实现回归82测试文件通过，246源码/28目录/110排除。保留36ff82f首轮CI35125290301的Windows22两项超时（8/9、整体失败）：后续证据提交未改实现而全量通过，不说明超时根因已定位或修复。
- 持久登录仍延期；Chat API确切后端身份是用户约定的后续任务。启发式结果提供参考，不承诺绝对鉴定。

- 当前续作：第28组修F27-02第三方卡片/指引/规则/资源跨端矛盾，未知字段不再误报免费或无隧道。正式清单198条中5篇逐句核对、3篇局部核对；本地82测试通过，第三方实机未验。下一项F28-01其余客户端/OAuth配对描述，再续R3其余模型/API/工作流；探测暂停，本批CI已恢复执行并九项成功，之前账户阻塞与Windows失败历史保留。
