...
- 当前产品代码47ee11c232e7c267bf56ac7a414e9bd77079d302（原生单文件草稿diff/受保护恢复），[CI35108769347](https://github.com/cccjvav/web_agent/actions/runs/35108769347)九项成功，含七组Node矩阵、安装器及经典工作台Chromium回归。本地80测试文件通过，242源码/28目录/110排除；新原生窗口diff/撤销仍待实机，不以Chromium代签。核心0.7.2、Companion0.5.2未改。公网首次失败/修正证据仍保留于阶段10。
- 持久登录仍延期；Chat API确切后端身份是用户约定的后续任务。启发式结果提供参考，不承诺绝对鉴定。

- 当前续作：入站/出站双角色与数据边界已补新手图解；发现并修正使用指南旧断言及模型可见external_servers漏公网的描述，测试/证据见阶段10。原生UX本批补单文件草稿diff/明确确认恢复，本批已实现选定文件的跨文件内容检查点；全项目/新建删除/外部副作用回滚不在这项能力内，逐句审查仍未完。
