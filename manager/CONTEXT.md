...
## 当前状态
- **R5第四包施工：真实Windows进程DACL拒绝回归。** 仅测试专用宿主/子进程ACL，目标不可访问与活宿主查询拒绝都必须不终止；不改产品入口或权限，真实矩阵待核验。图形/跨用户/实际PID复用仍待，探针暂停不变。
- **R5第三包：Windows本机交互预览/确认与稳定句柄回收已接入并经CI验证。** 仅新保护记录、标准隧道根进程、父PID/创建时间/映像匹配且宿主已死；活实例、未知及旧记录跳过。无图形按钮/API；ffb6f72真实Windows矩阵已过，ACL/跨用户与用户桌面仍待，详见阶段10。
