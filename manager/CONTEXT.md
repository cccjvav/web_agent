...
- **真实用户状态**：Windows桌面VS Code集成CMD中使用Conda，系统Node24；完整重启VS Code已解决npm/npx PATH问题。2026-09-14用户确认第5步复验无报错、第6步启动通过；第7步因空内置模型列表原先阻塞，修复后仍待用户复验（用户报告未另附日志/提交号/退出码数值）。当前会话未连接其本机MCP，未做桌面/UAC/DPI/剪贴板/手机验收。
