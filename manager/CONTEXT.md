...
- **仍待修（已取证未动）：** 无会话调用者合并已于F71修复。`reports.json`上限与轮转已于F70第五批完成（整日轮出最旧日期，不自动归档）；F61-05/06尚未独立复核。F54-04经实测已修且有既有测试覆盖（executionControl.test.js），可关闭。**仍未审**：`computer-use/win`（扩展F71第二批、admin-host与共享危险命令检测器F72已审）、`computer-use/win`（无dotnet/pwsh不可本地验）。`workbench/js`、`.github/workflows`、`mcp/server.js`、`installer/preparation.js`、`run-code-oss.js`已于第6–7批覆盖。下一批先复核本批相邻调用链，再按小包续修，不先扩功能。原F60源码ce68560/CI35662916656九项与交接63cbbdc/CI35663331086九项保留。
...
## 用户环境与最终验收
- 用户指定：剩余施工完成后，从Arena实际连接本机WebAgent MCP，选择本项目根；先核对工具/ping/workspace_info的身份/root、Git提交与现有改动，再受控测试。本机M1–M5未执行，停止后的进程结果仍须本机确认；沙箱不能代签。本轮逐步手册：[R8本轮Arena实机验收](../docs/guides/R8本轮Arena实机验收.md)（用户环境：Windows桌面VS Code＋集成CMD＋Conda＋系统Node，仓库根工作区，Quick Tunnel）。
