...
## 当前状态
- **R5第四包：真实Windows进程DACL拒绝回归已验证。** 仅测试专用宿主/子进程ACL，目标不可访问与活宿主查询拒绝都必须不终止；不改产品入口或权限；e6e9aac真实Windows矩阵通过，本地92/92，首轮夹具失败保留；后续befee7d Windows22原收据链失败（8/9），根因未定，进一步隔离测试线程令牌待核验。图形/跨用户/实际PID复用仍待，探针暂停不变。
