...
## 当前阶段
- **阶段10：继续上游借鉴与文档整顿**。Tasks隔离/刷新恢复与初步编辑预览（核心0.7.2）已交付；所有者权限与Chat/Bridge互斥已交付，当前继续文档逐句审查与旧指南退役、过期内容清理与实际缺口复核；见[阶段10](stages/s10-upstream-adoption.md)。阶段9工作区调整已交付。
...
- 当前产品代码348ce98a80af2bfe7fb81eba3a366a74ad51dfef（所有者权限与模式互斥），CI35097629168九项成功，含真实Chromium、Windows安装器和七组Node矩阵。本地78测试文件通过，237源码/28目录/109排除。核心0.7.2、Companion0.5.2未改；新控件真实Windows/手机人工验收仍待。
...
- 文档清理政策：过时使用指南删除或归档，不靠末尾追加新状态留在现行入口。已删除旧双向连接指南，当前有效步骤并入使用指南；早期探针接入报告移入review/archive，仅保留历史证据。
