...
## 当前状态
- **F63第三方复审消化+独立修复（本会话，分支arena/01a0cdcf-web-agent，基线26a167e）：** 逐项复核外部报告[web_agent_review_2026-09-23](../review/web_agent_review_2026-09-23.md)（P1/P2/P3+假阳性甄别：gitOps字节预算系9766c6c已修的旧基线误报），随后分批落地：①dangerousPolicy矩阵重写+~130条零误报测试；②apply_patch对已有文件的裸正文改E_BAD_ARGS拒绝（P1-1，红绿验证）；③Windows退出码三分支合同（`$__wa_ok=$?`+LASTEXITCODE，P1回归修复）；④sliceTextTail UTF-16安全截尾进ptyPolicy并四处共用；⑤P1-6 store不再改写受控.gitignore，改维护`.git/info/exclude`（hostPersist断言红绿）；⑥tunnelCleanupAclFixture MainModule NRE加重试（CI偶发35795513487根因，Windows CI待验证）；⑦permissions()按store.stateKey缓存、findCloudflared命中缓存/未命中30秒TTL（/api/status轮询热路径）；⑧listDir目录在前字母序（数字感知Collator）；⑨UI结构性高度px→rem（1.6×下状态栏垂直裁切实测复现并修复，Chromium实测取证）+窄屏状态栏可滑动+文件树truncated提示；⑩package.json元数据对齐（agent-host 0.7.2/main/src/index.js/license，extension副本同步）；⑪FULL_REVIEW_INDEX登记新报告至202份并刷新被改文档指纹。本地107/107全绿，docs重建一致。
...
- **R7 Markdown时效核对已完成（来自01a0c932，已合并）：** 以f317c44为基线枚举200份.md逐份判定现行一致/修正/待证据/历史/只读/生成/暂停，未留"待核对"；合并后加入本分支交叉验证台账，F63再登记第三方复审报告与review/README条目，现共202份。文档修正**不等于**外部兼容、实机或全仓源码安全认证。清单见[文档清单](../review/FULL_REVIEW_INDEX.md)。
- **仍待修：** `reports.json`无条数上限与轮转；F61-05/06尚未独立复核。F54-04已修可关闭。**F63补充静态扫描（非深审）**：bridge.js/operations.js的innerHTML全部escapeHtml、admin-host token用timingSafeEqual且坏存储fail-closed、computer-use/win脚本有窗口歧义拒绝与ERR_*合同——未见新缺陷但未逐行审，无dotnet/pwsh不可本地验；extension.js/ptyHost.js深审仍开放。`workbench/js`、`.github/workflows`、`mcp/server.js`、`installer/preparation.js`、`run-code-oss.js`已于第6–7批覆盖。下一批先复核本批相邻调用链，再按小包续修，不先扩功能。原F60源码ce68560/CI35662916656九项与交接63cbbdc/CI35663331086九项保留。
- **本轮验证边界：** F63本地107/107通过（沙箱真实Chromium 153实测UI裁切，仍无DPI/字体观感代签）。上一批104/104 + CI 35794970708 九job全绿（含Windows 20/22/24）；F63批CI结论待推后核对。**教训：本地全绿不等于没回归**，曾连推四个提交Linux绿而Windows全红；此后每批必查CI结论。沙箱下载CI日志被TLS阻断，改用`gh api .../check-runs/<id>/annotations`取真实断言。Chromium缺失且下载TLS中断，浏览器E2E与真实DPI/字体回退观感本轮未验，UI改动只有静态断言与DOM夹具证据。无本地Windows/C#/PS实机环境，旧CI不代签新反例或桌面。
