...
## 当前状态
- **R4历史Windows22超时追查：根因仍未定位，已补诊断首包。** 重新取到35125290301的annotations，确认echo在30519ms超时且无输出、worker在10秒startup超时；完整日志仍EOF。对照历史代码后只加默认关闭的阶段元数据、Windows CI启用及运行器结果元数据，不加时限/重试、不改进程保护。
