...
- **仍待修（已取证未动）：** 无会话调用者合并已于F71修复。`reports.json`上限与轮转已于F70第五批完成（整日轮出最旧日期，不自动归档）；F61-05/06尚未独立复核。F54-04经实测已修且有既有测试覆盖（executionControl.test.js），可关闭。**仍未审**：`admin-host/`（扩展已于F71第二批审）、`computer-use/win`（无dotnet/pwsh不可本地验）。`workbench/js`、`.github/workflows`、`mcp/server.js`、`installer/preparation.js`、`run-code-oss.js`已于第6–7批覆盖。下一批先复核本批相邻调用链，再按小包续修，不先扩功能。原F60源码ce68560/CI35662916656九项与交接63cbbdc/CI35663331086九项保留。
