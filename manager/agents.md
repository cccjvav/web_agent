...
R5用户授权安全隧道残留回收：首包只读归属/检测，后续清理须稳定句柄/可信归属/本机确认；不能把candidate、旧PID或进程名当终止授权，不得影响当前/其他活宿主。

R5收据完整性续包：Windows CurrentUser DPAPI不是应用签名，不能隔离同用户代码/管理员；旧v1不升级，不允许封装失败回退明文。清理仍禁用，后续必须稳定句柄与本机确认。

R5第三包：Windows本机TTY确认入口使用持有的原生句柄，只终止匹配旧宿主的标准隧道根进程；活宿主/不确定项跳过，不按PID重开目标或taskkill树，无HTTP/MCP新增权限。TTY/DPAPI不隔离同用户恶意代码。
