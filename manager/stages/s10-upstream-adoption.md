...
#### 第54组续批：ShunCode五问裁决与MCP会话两处修复（2026-09-20）

参考包只在仓库外`/tmp`解包读取，未安装未执行。两处修复均先真实负例后最小修复：

- 重复`Mcp-Session-Id`头：raw socket发两个同名头，Node合并为`"sid, sid"`，旧实现用合并串查会话必失配→404“会话不存在”，客户端被误导重复initialize。现POST/GET/DELETE在会话查找前对数组形态或含逗号/空白的会话头统一400/-32600；单个未知ID保持404/-32001。修复后真实HTTP复测dup→400、single→200。
- 在途会话驱逐：红测证明容量满时`createHttpSession`按lastSeen驱逐最旧会话，不看是否有请求在飞；TTL prune同理。长工具调用（可到5分钟超时）期间被驱逐后，该会话的取消通知和后续调用失去身份绑定。现`beginHttpSessionWork`在POST处理期与SSE打开期登记在途计数，prune/驱逐跳过active>0，全在途才兜底驱逐全局最旧；release一次性、未知ID为no-op。
- 不采用项：JSON-RPC ID登记（现有requestLifecycle按owner+带类型id隔离，比ShunCode全局Set更强）；自适应并发（无负载证据，现有64在飞/32 SSE上限充分）；事件缓冲/断线重放（无流中结果可重放，其实现自认全局有界挤占缺陷）；文件工具确认不替换（无dryRun/hash/检查点/审批耦合，schema校验静默放过`$ref`等）。
