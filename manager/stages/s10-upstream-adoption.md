...
当前任务：第29组统一管理职责并修F28-01客户端/OAuth指引；下一工作包以manager/ROADMAP为准，R2/R3其余项继续。文档逐句语义审查仍进行中，范围与实证见[文档台账](../../review/SEMANTIC_REVIEW_2026-09-16.md)。以下先记录已交付批次，不能把历史测试数当最新结果。
...
第29组本地完整82测试文件通过，246源码/28目录/110排除；生成/构建、管理入口/全量文件登记/链接与协议/HTTP/VM回归均通过。精确提交CI推送后核对，不继承第28组结果。
