...
| 既定施工部分完成 | 08编辑预览与受保护回退 | dryRun/diff/hash及经典已有文件草稿预览/显式确认已实施；跨文件/原生UX与保护回退仍未完成 |
...
API/编辑器VM/浏览器流程增加回归；本地78测试文件通过，237源码/28目录/109排除、文档构建与diff检查通过，CI待本批提交后核对。余项继续按表推进，不重新开启已取消入口。
