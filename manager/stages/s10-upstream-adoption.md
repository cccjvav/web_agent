...
新增模拟协调器确认单次/过期/来源漂移/重复帧/取消/管道拒绝；Windows真实测试复制Node为同名伪隧道，测活宿主、父PID/出生/映像不符、同名无关进程保留、持有期间退出、来源变化及确认后观察终止。旧owner夹具只增可选程序路径，默认程序/60秒自退保持。Linux定向通过，不代签原生Windows；全套/精确CI待核验，用户桌面/真实隧道未验。R4根因及其他工作包/暂停探针不关闭。
