...
5. **当前施工与下一步**：本轮先红测并修run-tests.js的head/tail annotation漏阶段问题，只增加白名单生命周期摘要；代码7f62350246f682c5f1c23f912a1206900904d7db / CI35620755160已核实九项、本地94/94；本段后续记录提交仍须核对自身HEAD，不继承代码SHA绿灯。先核对对应tests/说明/精确CI。随后优先在新出现的真实Windows失败里分析spawn/ready/输出/退出；若新证据只显示spawn无输出，下一步才对Add-Type/Attach的内部阶段设计不污染工具输出的取证；无可解释复现时不改根因，不加时限、删断言或重跑追绿，也不暂停全部项目等待偶发失败。R2/R3其余合同与全仓逐句继续，不宣称只剩实机。
