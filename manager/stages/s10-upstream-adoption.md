...
| R6 / 下一项（用户2026-09-25选定：VS Code插件一体化启动返工，[方案稿](../../docs/development/插件一体化启动返工方案.md)待用户确认D1–D4后施工；网页工作台保留） | 候选设计与分项实现 | 第4.2节、上游26类地图；完成明确缺陷修复优先 | 每项先写最小范围、输入/预算/权限/失败、回归与取舍；有收益且不突破授权边界再落地，不把全部候选统一许诺为必做 |
