...
| R6 / 下一项（用户2026-09-25选定：VS Code插件一体化启动返工，[方案](../../docs/development/插件一体化启动返工方案.md)D1–D4按推荐确认；**第一期已实施待实机验收**（[验收手册](../../docs/guides/插件一键启动实机验收.md)），用户同意与第二期并行；第二期设置页进行中，第1批请求收拢、第2批扩展转发层已完成（方案第9节）；网页工作台保留） | 候选设计与分项实现 | 第4.2节、上游26类地图；完成明确缺陷修复优先 | 每项先写最小范围、输入/预算/权限/失败、回归与取舍；有收益且不突破授权边界再落地，不把全部候选统一许诺为必做 |
