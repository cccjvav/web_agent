...
### 第83组：R6第二期第4批——外部MCP迁入设置标签页（2026-09-26）

**开头复审（第82组）：** `ace44f2`的CI 36267718025九job全绿。复读：接入列表`#ops-servers`及其“移除”按钮在第3批的隐藏包裹内，待批请求列表在包裹外且批准已在白名单；`readHostLocation`各错误路径都抛出、由调用方捕获。未发现缺陷。

**用户决定（D4逐项确认，经ask_user）：** 外部MCP**迁入**设置标签页；多模型博弈**留在网页工作台**，理由写进方案与手册，将来若做VS Code支持另立“插件Chat分支与总结”一项。原第4批（验收手册）顺延为第5批。

**本组：** `extension/apiRelay.js`白名单加POST `/api/external/servers`、DELETE `/api/external/servers/<ID>`、POST `/api/external/stdio/preview`与`/stdio/start`；`/api/external/request`（本机工具调用，工作台不调用）与`/api/consensus/*`继续拒绝，头部注释改写。`workbench/js/operations.js`：公网HTTPS登记与stdio启动由原生`confirm`改为`await confirmAction`；stdio启动改为弹框**前**置忙并禁用预览/启动按钮（原先确认后才置忙——浏览器里原生框阻塞无妨，异步框下连点会弹两个框），确认后复核current，取消恢复按钮。`index.html`去掉包住登记字段的`<div data-workbench-only>`。`settings-panel.js`导航名“审批与检查点”改为与页面标题一致的“工具接入与审批”。已安装副本同步apiRelay.js与入口详解。

**测试：** settingsRelay（放行四个接口；拒绝23条，新增GET servers、PUT stdio/start、无ID的DELETE、stdio/stop、带斜杠ID）；settingsPanel partA（operations页整段无隐藏标记，共识仍有）；workbenchRuntime末尾新增标签页异步确认一段，原有两处“点击后立即读计数/定时器”的断言改为先等一轮并断言请求已发出；workbench.browser的settingsPanelBrowser（签名加workspace）断言登记字段可见，并用真实主机与stdio夹具走预览→VS Code对话框取消（夹具未运行）→确认（恰好一次、夹具运行）→列表→经转发DELETE移除。反向验证：单元18种、浏览器5种变异全部变红。其中“重新包上`data-workbench-only`”第一版只插了开标签，红在别处的超时而非目标断言；改成成对包裹后红在“`#ops-name` is offered in the tab”，计入的是后者。

**交付前自查（逐项）：** ①读完整diff与调用方：转发ID规则`[A-Za-z0-9_-]`容纳UUID，主机仍校验UUID版本；stdio启动40秒期限小于转发120秒；移除在浏览器里本无确认，两边一致；工作台里仍用原生confirm的只剩tabs.js（标签页没有编辑器页签）。②文档声明逐条对源码：初稿写“stdio程序继承主机环境（系统Node）”，核对`stdioLaunch.js`后更正为只得到BASE_ENV（PATH、PATHEXT、SYSTEMROOT、TEMP等）与配置里的env，三处文档同步。③失败/取消/外部中断后的残留：弹框期间取消、编辑、换工作区、预览过期都不发送且按钮状态可恢复；stdio启动在途时关闭标签页，转发层dispose取消HTTP，但主机可能已启动进程——重开标签页后接入列表可见、可移除，与浏览器里关页相同，记为剩余风险。④用户场景：Chromium中以插件真实的页面改写、转发层与服务处理器连接真实主机验证；真实桌面VS Code对话框与Windows进程释放未验证。⑤新断言逐一反向验证（见上）。

**环境插曲：** 本轮中途沙箱重启，HEAD回到`4a868ae`、文件保留、`/tmp`与部分依赖丢失（settingsRelay报找不到express）；按既定办法fetch＋`reset --mixed`回到`ace44f2`，`npm ci`、重建变异脚本与Chromium后继续，工作区改动无损。
