...
新测试用可控authority检查明文可信标签伪造/密文替换/改名/损坏/失败不降级/迟到退出；Windows专门运行真实DPAPI往返中文/emoji、翻转密文与预算失败，原真实孤儿/活实例夹具保留，发行断言新增JS及PS辅助。Linux三项定向和全套89/89通过，文档260/28/110及185登记指纹同步。文档守卫初次因PS脚本错放JS映射而失败，现放回非JS artifactPairs，保留原守卫。精确Windows CI待核验。探针继续暂停，R4根因等其余工作包不关闭。
