...
1. **定位与保护工作区**：先查git status --short、git log -1、git branch --show-current。当前会话绑定arena/01a0c5ba-web-agent，起点bbe79854已与此前arena/01a0c4b1-web-agent一致；历史快进来源仍是arena/01a0bfa9-web-agent的67f4f966720c2d6f657f53e3dc3b02cbf3860b98，[CI35624122815](https://github.com/cccjvav/web_agent/actions/runs/35624122815)九项与本地94/94为进入基线。F55证据提交036d65b / CI35632714383、F56源码a68a77e / CI35641420076及证据4649250 / CI35642269647、F57源码4cf4d6f / CI35647781757及证据ee393c2 / CI35648274576、F58源码6daf576 / CI35651156739均已逐job核实九项；不覆盖新增改动或机械切换历史分支。
...
5. **当前施工与下一步**：F59修正可选编辑器未等待ensure结果的缺口，并让打开窗口后的身份再确认、准备阶段保留子进程取消与说明一致。复审发现两处残留：超时与取消会覆盖升级计时，打开后的短暂探测会被当成身份变化并停掉自有窗口。F60只修这两处，本地验证见第60组，不能代签用户桌面。下一线索仍是更细的资源/权限边界，以及Windows `shell:true`可能留下的npm孙子进程；不按名称或端口补杀。200/healthz不能代签身份。R4历史原因仍开放，不加期限/删断言/重跑追绿；R5真实后代/窗口、R7逐句和R8用户验收继续。
...
| R3 / 高，继续 | 第25/27/31–37与41–43/45–53组持续修复消费链。第53组已补齐所有当前非Probe路由的query门禁、external/workflow显式固定接线、status/diagnostics与定制/会话投影；F54第三批已补新文件patch显式hash与块校验，第五批补原生postNdjson坏流/终态及失败历史；F55补经典流严格完成/预算/取消清理；F56补可选编辑器编排健康期限与直接子进程收尾；F57补App窗口身份绑定与浏览器失败；F58补同步准备阶段超时/取消，F59补ensure等待、打开后身份再确认与保留子进程取消说明，F60修正准备计时覆盖和打开后短暂探测被误判，按新证据续修，不重做已交付链 | [API逐项详解](../../webagent-core/agent-host/src/api/路由逐项详解.md)、routes、apiFiles及已登记消费者；明确排除探针专项 | 每路由核对HTTP与业务结果、请求/响应预算、审批前后复查、deep copy/幂等/取消/unknown；失败不自动重放，不扩大任意命令权限，脱敏凭据不能转绑新连接 |
...
| R7 / 结构首包已做，语义继续 | 第26组集中19篇专题、归档17篇旧审查、删除过期PROMPT；F54改相邻错误说明、归档独立报告与同步当前基线，F55修文档站跳转/窄屏导航/代码/对比度，F56同步启动/运行器说明并纠正旧mkdir/副本版本，F57拆分appWindow并加身份/浏览器回归，F58补准备阶段超时/取消，F59同步ensure等待、打开后再确认与样式说明，F60同步准备计时与打开后重试说明，均未增加整篇逐句完成项；其余README/管理旧现状继续核对 | 源码清单、目录README、根维护/安装说明及阶段索引 | 活跃正文无相互矛盾的“当前”；无用旧指南退役，有效教学/历史失败保留；给出已审和未审清单而不是总称100% |
...
下一位助手先看本节即时接手检查和最新批次。非Probe query、只读投影及external/workflow接线已在第53组交付，不再当尚未施工；F54至F60已修的会话/原生/经典流/响应头/启动确认/直接子进程/准备取消/打开后再确认及准备计时覆盖也不重做。继续按R2/R3的新负例推进（当前待验证线索为更细资源/权限边界，以及Windows npm shell孙子进程不补杀的剩余核对），交叉R4与R7；无需用户重新复述此前授权和约束。如发现与实际代码不符，以核验结果修订交接，而不是照抄本页当绝对真相。
...
### 第59组：ensure等待、打开后再确认与说明对齐（2026-09-21）

沿第58组待验证线索复核外层身份与准备取消。可选编辑器`run-code-oss.js`调用已改为async的`ensure`时没有await，会把Promise当成入口路径；现等待结果后再取目录并启动编辑器。打开窗口前的身份核对之后，再在浏览器已打开时复查实例；不匹配不把本次当干净移交。准备阶段超时/取消只向本次保留的npm/下载子进程发信号，不按名称、端口或旧PID补杀。

- 本地完整主机套件97/97退出0。首次全量唯一失败是发行副本未同步已改的扩展详解，按字节复制后重跑仍为97/97；没有删断言。文档277源码/28目录/111排除，重建后updated=0。不是用户桌面或真实code-server验收。代码`bbf22e7a5924820d32ef1341de97f1983cc268bc` / [CI35657576065](https://github.com/cccjvav/web_agent/actions/runs/35657576065)已逐job核实九项success（Ubuntu Node18/20/22/24、Windows Node20/22/24、Windows安装器、真实Chromium）。记录该结果的后续文档提交须另核对其自身HEAD。
- 说明原地对齐：安装器函数详解、编辑器编排、运行器测试详解，以及工作台/文档站/扩展的辅助字号与空状态颜色。发行副本与源码字节一致。正式清单仍是已逐句8、局部69、待66，不增加整篇通过数。
- Windows `shell:true`可能留下未跟踪的npm孙子进程，本批只在说明中标明，不新增taskkill。更细的资源/权限边界仍待。R4/R5/R7/R8与探针暂停不关闭。

### 第60组：复审F59的计时覆盖与打开后误判（2026-09-21）

F59已交付并有九项CI，但复审源码后确认两处残留，不是交接叙述里的未决猜测。

- `ensureDependencies`与`runNpm`在超时和取消重叠时，后一次直接覆盖`forceTimer`/`giveUp`，已结算后旧计时器仍可能再发SIGKILL。放弃时也没有unref仍活着的句柄，启动器可能在拒绝之后仍被挂住。现由`armEscalation`先清上一轮计时，失败时`releaseLive`只unref尚未退出的保留句柄。unref不是退出证明，不新增taskkill，不回收Windows `shell:true`的孙子进程。
- 打开浏览器后，一次非ready探测被当成身份变化，catch再`stopOwned`自有supervisor，短暂拒绝会关掉刚打开的窗口。现只在同一120秒期限内重试waiting/absent；hostIdentity抛错或ready但identity与pin不一致才不移交。不终止另一服务。

定向回归覆盖超时放弃unref、超时窗口内取消后旧计时不再发信号，以及冷启动打开后一次主机拒绝仍移交、真实身份变化仍只停自有supervisor。未执行真实npm、code-server下载或用户窗口。本地完整主机套件97/97退出0，文档277源码/28目录/111排除且updated=0，生产audit 0漏洞。先前文档提交`a2085766ddefc71c59b58ec83b2b98614d9cb511` / [CI35657937517](https://github.com/cccjvav/web_agent/actions/runs/35657937517)九job已核实，只覆盖F59说明，不代签本批。精确提交与CI按本批HEAD另核，不用`bbf22e7` / CI35657576065代签。正式清单不增加整篇通过数。R4/R5/R7/R8与探针暂停不关闭。
