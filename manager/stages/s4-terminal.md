...
- UI 参考资料：用户 2026-09-07 补发 18 张 ShunCode 截图（17 张不重复）。**注意：附件轮字节曾未落沙箱盘；用户随后两批网页上传入仓（`758532a` + `176cabf`）共 18 张，全数归档 `review/shuncode-ui/`（含附件轮缺的 2 张与新增「Bridge 等待态」；重复原件已去重）。**
...
## 子计划
 ...
 - **S4-4 资料补齐（完成）**：两批网页上传共 18 张（`758532a` 16 + `176cabf` 补 2）齐；归档 `review/shuncode-ui/` 去重（16 个重复原件 git rm）+ 重命名 + 索引 README。归属纠正：12/13 曾误名 `chatgpt-*`，弹层实为 ShunCode 自有模型弹层（ChatGPT 客户端仅内嵌背景），改 `shuncode-*`；命名与归属约定见夹内 README（本项目是 Web Agent，ShunCode 仅参考）。
...
- [x] review/archive/REPORT_SHUNCODE_S4.md 已写（含真机验收清单：.iss 编译/安装卸载冒烟/弹层明暗/branch-pill/等待态文案对照 05 图/S3 遗留远程实收 image 与 >6MB 降级）
- [ ] S4-4 截图字节入 review/shuncode-ui/（用户自行入仓后 fetch 核对）

## 阶段结论（2026-09-07）
代码面完工并关账：S4-1 安装包 `4a85ecf`、S4-2 默认壳口径 + S4-3 抛光 `8a73df0`、S4-3 弹层 `ce6d52f`、S4-4 截图归档 `277cb71`、收尾报告见 `review/archive/REPORT_SHUNCODE_S4.md`；真机验收唯一基线=`review/CHECKLIST_WINDOWS.md`（D 节 D1–D8，2026-09-07 由 S4 清单与 V 时代清单合并）。真 PTY 未点名不做（§12 取舍保留）。后续仅真机验收反馈驱动小修。
