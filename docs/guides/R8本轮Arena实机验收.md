...
**预期：** 日志里的 Workspace 是你的仓库根。浏览器打开`http://127.0.0.1:3000`。VS Code右下角状态栏应是`Web Agent`（不是“工作区不一致”或“未连接 48271”）。
...
1. 工作台Bridge：隧道选 **Cloudflare Quick Tunnel** → 启动Bridge。成功后应出现`https://…trycloudflare.com`域名；仍是`127.0.0.1`就停下（见[使用指南](../../使用指南.md)的Bridge故障说明）。
2. 复制MCP地址，**只填进Arena的MCP连接配置**（与你之前手机/电脑成功接入的方式相同），不要贴进聊天。Quick Tunnel每次启动域名都会变，旧连接要换新地址。
