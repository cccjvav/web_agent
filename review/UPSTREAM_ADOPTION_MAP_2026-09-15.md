...
| 04 | 真正接入第三方MCP | mcp_client/client.py:list_tools/request/stop（前后两批抽查） | 已有回环HTTP与单次审批；上游list_tools此处也只读单页，不应称它实现了分页 | 本批自行补有界分页、总预算/总截止时间、深拷贝目录、严格分片SSE；stdio与公网接入仍待做 |
...
| 08 | 安全编辑的预览/确认/回退 | files/safe_edit.py:build_edit_preview/create_preview/apply_preview（前150行） | 我们已有hash冲突、补丁与审批；上游应用前比对原内容、预览有TTL | 候选：审批中展示可读diff及基线hash；回退也必须检查当前版本，不能无条件覆盖。未认证其并发回退安全 |
...
| 11 | 限流恢复与公平性 | rate_limit.py（全文） | 已有MCP入口边界与资源限制，但不能由限流名称推断有会话公平性 | 候选：拒绝请求不延长封锁窗口、明确Retry-After、全局状态键上限；需真实网络代理/会话区分测试，不按不可信头任意取身份 |
...
| 18 | 变异/性质/模糊测试 | pyproject.toml的dev依赖；.github/workflows/mutation-sweep.yml（前65行） | 已有跨平台真实回归，尚无性质/变异门禁 | 候选：先对路径、SSE分帧、hash冲突、审批幂等写生成用例；昂贵变异单独手动任务，不复制小时级全仓门禁 |
