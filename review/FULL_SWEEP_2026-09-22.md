...
## 追加：S10–S15（2026-09-22，经对方 F61 自审报告触发、本分支独立复现）

对方助手在其分支上传自审报告（COMPREHENSIVE_AUDIT_2026-09-22.md，F61）。其附录复现脚本经我在其 HEAD `2f6e7ab` 与本分支 HEAD 分别执行，六项发现中五项在本分支**直接复现**、一项静态确认同在——均为共同基点遗留，列为本分支缺陷清单补充：

| 编号 | 对应其编号 | 本分支实测 | 内容 |
|---|---|---|---|
| S10/P1 | F61-01 | 复现：explicitDenied=true 而 default/staged diff 均泄露标记 | git_diff 无 filePath 时绕过逐路径敏感检查，整体 diff 可带出 .env 等被禁内容 |
| S11/P1 | F61-02 | 复现：字节 41 ff 0a 与 41 fe 0a 读取 hash 相同，旧 hash 写入 success | 非法 UTF-8 被替换字符吸收，hash 基于解码串而非原始字节，过期写保护失真 |
| S12/P1 | F61-05 | 复现：8000 行 diff 2007ms 仍未完成被看门狗终止 | createUnifiedDiff 同步无计算预算且同一差异算两遍，长 diff 阻塞事件循环 |
| S13/P2 | F61-03 | 复现：fetch 无 signal、父取消后仍返回用户、>1MiB JSON 被接受 | GitHub 身份三链无应用层取消/字节/期限合同 |
| S14/P2 | F61-04 | 静态确认（admin-host 两分支零 diff） | 统计后台坏 JSON 当空表、下次上报整文件覆盖原字节 |
| S15/P2 | F61-06 | 静态确认（本分支同结构更旧） | run-code-oss fallback npm install 无独立 deadline |

另两分支共有微小项：computerUse.js/server.js 注释指向已归档的 review/REPORT_SHUNCODE_S3.md（=其 D2）。

对照验证全文与对其自审报告质量的裁决见[分支对照报告追加二](BRANCH_COMPARISON_01a0bfa9_2026-09-21.md)。本追加只登记发现；S10–S15 的修复同样以"优先收敛"为前提，若决定本分支单修，其报告第三节的修复验收门槛可直接复用。
