...
| [manager/CONTEXT.md](../manager/CONTEXT.md) | 已修正 | 4a034c4fe6183601 | 按F67实际进度更新入口/基线和.md分母；旧CI只作对应提交证据，实机/暂停/遗留单列。 依据：本轮清单、Git HEAD、阶段10与用户更正 |
...
| [manager/stages/s10-upstream-adoption.md](../manager/stages/s10-upstream-adoption.md) | 已核对一致 | 44a2163ab67964d8 | 当前唯一工作包表/失败/批次与F66–68实际进度一致；历史段按时间保留，R7仍为下一项。 依据：CONTEXT; 本轮200文档清单; 实际提交/测试/CI记录 |
...
| [webagent-core/agent-host/src/mcp/README.md](../webagent-core/agent-host/src/mcp/README.md) | 已核对一致 | dc6da071dae8b289 | 核对入/出站角色、版本/批量/SID与公开peer、权限/取消、OAuth/SSE和软预算；平台/多租户/第三方边界不扩大。 依据：mcp/server/session/oauth/requestLifecycle/budget; MCP回归 |
...
| [webagent-core/agent-host/src/mcp/请求分发详解.md](../webagent-core/agent-host/src/mcp/请求分发详解.md) | 已核对一致 | c2dc3de9dc06c6f5 | 逐入口核对认证/版本/SID/整批预检、pin、结果层、资源/提示与64在途/5分钟取消；旧兼容与协作取消限制准确。 依据：server/requestLifecycle; mcpProtocol/mcpBoard/mcpCancellation/requestLifecycle |
...
| [webagent-core/agent-host/src/tools/README.md](../webagent-core/agent-host/src/tools/README.md) | 已修正 | 1fbd684f837d2395 | 修搜索启动10+扫描2秒和write_file覆盖条件非强制双重AND；文件/工具/缓存/PTY/协作边界已对照。 依据：tools/index/fileOps/patchEngine/readCache; worker和文件回归 |
...
| [webagent-core/agent-host/src/tools/工具入口与命令策略详解.md](../webagent-core/agent-host/src/tools/工具入口与命令策略详解.md) | 已修正 | 0aedf1fde51fa10c | 去旧固定工具数、统一dispatchTool与callTool追踪/核验分工；模式、别名、remote ACL和词法危险规则与源码一致。 依据：tools/index/normalize/dangerous; extension/dangerousPolicy; 回归 |
...
| [webagent-core/agent-host/src/入口详解.md](../webagent-core/agent-host/src/入口详解.md) | 已修正 | 85e101f3d0ca9b2a | 消除第1步已优先检查根却又说不能保证先检查的旧矛盾；核对装配/监听错误/健康/WS/shutdown8秒边界。 依据：src/index.js; workspaceEntry/httpSmoke/skipWorkbench |
...
| [webagent-core/agent-host/tests/MCP协议与整机入口测试详解.md](../webagent-core/agent-host/tests/MCP协议与整机入口测试详解.md) | 已核对一致 | 64db3a00c950484c | 协议/HTTP/容量/审批等测试入口和隔离边界匹配，未把真实服务代入。 依据：documentationLearning具名函数/主说明守卫；本轮完整101/101；相应定向测试。 |
