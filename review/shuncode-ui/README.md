...
**本项目是 Web Agent（webagent）**。本夹存放的是**参考产品 ShunCode** 的界面截图（源自同名 bilibili 频道视频帧，用户 2026-09-07 两批网页上传入仓：`758532a` 16 张 + `176cabf` 补齐2张，前两批共18张；下述第三批再加7张，当前索引合计25张；与原归档重复的原件已 `git rm` 去重）。**参考归参考：产品命名、文案归属一律以 Web Agent 为准**，本夹只作设计对照，不抠图、不复制素材、不引入 vendor 代码。
...
**历史参考的推断边界**：这些画面曾被用于推测其与Code-OSS/Electron、Inno Setup的相似性，但界面、体积和许可页不足以证明第三方的实际运行时或打包实现；当时没有关于页、安装目录或可验证源码，不能写成“必为Electron”。本项目采用Inno的事实来自自己的installer/webagent.iss，不由参考截图替第三方背书。截图只保留设计观察和原来源，不新增兼容性、许可充分性或实现真实性认证。

## 隐私说明
截图含历史trycloudflare临时地址（当前有效性未验证，仍按敏感信息对待）、账号名和他人本机路径（如视频作者工作区目录）——作为历史参考保留；**不要**把其中任何 URL/路径/令牌抄进文档或代码。
