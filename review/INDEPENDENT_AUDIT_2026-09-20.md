...
> 阅读顺序：第1–6节保留2026-09-20的F54历史观察；2026-09-21的浏览器/界面修复见第7节F55，后续启动可靠性见第8节F56，身份与准备取消见第9–11节，第12节只修正F59复审发现的两处残留。当前工作包只在阶段10维护。
...
## 11. F59 ensure等待、打开后再确认与说明对齐（2026-09-21）

本节不改写第1–10节的历史观察。当前会话固定在`arena/01a0c5ba-web-agent`，开工前确认它与`arena/01a0c4b1-web-agent`的`bbe79854`一致，没有切到其他分支。先读manager与既有review，再对照当前源码。

### 已对齐的行为

| 编号 | 发现 | 处置与边界 |
|---|---|---|
| F59-01 | `run-code-oss.js`在`ensure`改为返回Promise后仍同步使用返回值，入口可能是Promise而不是路径 | 改为`await ensure`后再取目录并启动编辑器。回归要求入口参数等于准备好的`entry.js`。不是真实code-server下载或窗口验收 |
| F59-02 | 打开浏览器后没有再次核对实例，可能把已变身份当成干净移交 | 浏览器打开后再`inspectPair`；不匹配使用既有“窗口可能已打开，未移交后台”，不终止另一服务。当时把非ready也当成变化，F60已另修 |
| F59-03 | 准备阶段超时/取消若只靠`spawnSync`，不能向本次子进程发信号 | 保留本次npm/下载句柄，超时与取消只向该句柄发信号。不按名称、端口或旧PID补杀；Windows `shell:true`可能留下未跟踪孙子进程，本批不新增taskkill |

工作台和文档站辅助字现为12px；扩展空状态不再使用硬编码`#6e6e6e`，改用主题的`descriptionForeground`。这不是WCAG认证，也没有新增浏览器字号断言。

### 未关闭

R4历史Windows超时、R5真实后代/窗口/跨用户、R7全仓逐句、R8用户本机MCP与IDE仍开放。探针专项继续暂停。精确提交与CI以阶段10第59组后续记录为准，不用F58绿灯代签。

## 12. F60 复审残留：计时覆盖与打开后误判（2026-09-21）

本节不改写第11节当时的验证事实。复审已提交的F59实现后，确认两处会在真实路径上出错，并已最小修复。

| 编号 | 发现 | 处置与边界 |
|---|---|---|
| F60-01 | 超时与取消先后安排升级计时时，后一次覆盖句柄，旧计时器在结算后仍可能SIGKILL；放弃后不unref，启动器可能被活句柄挂住 | `armEscalation`先清上一轮计时。失败时`releaseLive`只unref尚未退出的保留句柄。unref不是退出证明。不新增taskkill，不回收shell孙子进程 |
| F60-02 | 打开后一次waiting/absent被当成身份变化，catch对自有supervisor调用stopOwned，短暂拒绝会关掉刚打开的窗口 | 同一120秒期限内重试非ready。只有身份抛错或ready但与pin不一致才不移交。不终止另一服务 |

定向回归先锁住旧计时不再补发信号、放弃后unref，以及冷启动打开后一次主机拒绝仍移交。不是真实npm、code-server或用户窗口验收。精确提交与CI见阶段10第60组，不用F59绿灯代签。R4/R5/R7/R8与探针暂停不关闭。
