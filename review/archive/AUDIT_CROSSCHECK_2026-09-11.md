> 历史归档：仅保留当时的发现、授权与证据；其中“当前”、待办和Git操作不作为现行指令。现状见[管理索引](../../manager/CONTEXT.md)与[路线图](../../manager/CONTEXT.md)。

# 多报告交叉验证与修复台账（2026-09-11）

**输入基线**：`049b08e`（用户上传报告；产品代码与 `7c9bde5` 一致）。
**外部报告来源**：原归档project_audit_report.md已于2026-09-16去重删除，原文仍可查Git历史；同组发现的[保留对照稿](01a08d85-web-agent-audit.md)仍在。两稿标题和部分措辞不同，不冒称字节相同，也不因去重认可其结论。
...
| F23 | 隧道stop/start进程对象及exit竞态 | 第七批代码及进程fixture修复；真实隧道/Windows进程树仍待 |
...
| F27 | 全量同步文件读取/正则回溯阻塞 | 第七批有界读取及可终止worker；保留正则支持 |
| F28 | budget裁切破坏schema与cursor | 第七批保留schema/游标；软预算与工具硬边界分离 |
...
| F37 | computer-use返回失败但脚本报OK、焦点/坐标/剪贴板 | 第八批错误传播/目标唯一/坐标焦点/剪贴板代码修复；Windows桌面验收仍待 |
| F38 | Plan全局轮次await期间更换，结果串任务 | 第五批代码修复＋回归；详见第十节，真实客户端/平台兼容仍待 |

文档待办也保留：D01主题文档本轮更新；D02第八批补summary实际内容及文件页锚点跳转；D03安全/审批承诺本轮仅同步已修项；D04当前分支/管理状态/历史计数本轮标识；D05第四/七批安装打包和卸载保留策略已实现，实机验收待；D06测试能力本轮补真实模块、HTTP/WS及runner测试，但完整浏览器/平台矩阵未完成。
...
## 十二、第七批：隧道与资源预算（验证进行中）

隧道启停以进程引用和 generation 绑定；替换必须等旧进程退出，过期输出/退出不得覆盖新状态，取消未就绪启动，停止失败拒绝替换。补进程事件 fixture；不等同真实公网/Windows进程树验收。发行白名单补 Add-Type 所需 `.cs`。

F27：文本读/写/补丁结果8MiB限制，批量读取20路径上限；目录列表1000条扫描/深度8限制。搜索移入最多4个worker，2秒deadline及取消终止，保留既有扫描预算；不再把启发式正则检查视为充分保护。F28：预算改为软字符目标，保留类型、数组成员、控制字段及分页内容，不再伪造nextCursor或截断序列化JSON；超预算标识 `_budgetExceeded`，非分页文本可显式截短。硬资源限制由各工具实施，不能宣称每个响应严格16k。find_files补双星目录零层匹配。

第七批最终全量48/48测试文件通过，退出码0；文档站重建通过。资源回归覆盖分页保留、超预算标识、有界文件读取、worker取消、目录限制及根目录glob。


## 十三、第八批：Windows动作结果与文档路由
F37：输入脚本错误不再报OK/exit0，窗口子串须唯一；C#检查可检测焦点/坐标/PostMessage失败；输出SUBMITTED不冒充应用成功。type采用可用格式快照及finally恢复，外部剪贴板变化拒绝覆盖，恢复失败明确错误。焦点竞争、DPI及特殊剪贴板格式需要Windows桌面验收，不能保证绝无错目标。
