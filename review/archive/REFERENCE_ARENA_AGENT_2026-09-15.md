> **历史归档，非现行指南/待办。** 本文保留当时源码观察、授权取舍与精确提交/CI证据；旧“当前”“最新”“未完成”以当时基线为准。11.3已由用户关闭，手机连接已另行通过，公网HTTPS与经典单文件回退已有后续实现。现行范围见[管理索引](../../manager/CONTEXT.md)及[逐句审查台账](../SEMANTIC_REVIEW_2026-09-16.md)。不篡改原始失败记录，也不拿旧建议覆盖现行权限。
