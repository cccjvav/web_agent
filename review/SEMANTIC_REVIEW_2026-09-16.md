...
第36组F36：HTTP登记连点两POST、removed:false被消费为成功先红测复现后修；表单快照/操作键互斥、独立结果与固定异常提示、移除停止未确认。允许移除connecting且旧登记回包不覆盖新移除；真实HTTP服务仍活着及stdio最终退出分别验证，后端合同未改。本地82测试/文档生成/构建/一致性通过，新增Chromium待精确CI；仅局部审查，下一包stdio预览/启动，R2/实机/全仓与暂停边界保留。
