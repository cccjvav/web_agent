...
第41组F41：经典reset-secret回调HTTP500仍报成功先红测后修；页内确认/互斥、绑定与旧secret条件比较、完整响应与后读失败分离，未知不重放。真实HTTP覆盖零副作用拒绝/并发一成功、保存前失败保留旧key、保存后失败已轮换及旧值重发409；旧扩展空体兼容，不代签原生命令UI。新增浏览器合成场景本地未执行；82本地测试/文档检查通过。首轮CI35309852333为8/9，浏览器夹具未开设置弹窗导致按钮不可见；修真实导航，保留失败、不强制点击，修正提交CI待核验。仅局部，197项逐句7、局部30、待逐句103；下一包Bridge启停与原生重置命令，其余待办/暂停边界保留。
