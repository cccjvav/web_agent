...
第34组F34-01/02：先复现检查点GET失败阻断审批GET、错ID预览仍授权；修为两列表独立校验/代次发布、检查点完整预览与恢复逐文件合同复核，未知不重放。真实写后事件异常证明后端既有unknown/停止后步正确，不冒充新增修复。本地82测试与文档生成/构建/一致性通过，新增Chromium场景待精确CI。正文只扩大相关局部，不关闭全仓逐句、R2或实机；探测保持暂停。
