...
**不知道先做哪一项时：** 打开[Windows新手逐步验收](../Windows新手逐步验收.md)，从第1步开始。它提供VS Code集成CMD、逐条命令、预期结果、失败停止点和可选MCP辅助路线；本文仍是唯一通过标准。可以暂缓自动测试以便先做最小连接，但必须记录未执行，不能因此认定已完成安全验收。
...
前置是剩余施工完成，由用户接入本机；源码根工作区，不使用沙箱替代。逐步操作见[最终本机MCP验收](../Windows新手逐步验收.md#最终本机mcp验收以本项目根目录检查产品)。
