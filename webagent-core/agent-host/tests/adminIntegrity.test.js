...
// NOTE(merge 01a0c932): this branch raises E_STORE_CORRUPT (carrying the file path and the
// reason) where 01a0c932 raised E_REPORT_STORE. Same fail-closed contract: a damaged store is
// never treated as empty and the original bytes are left exactly as found.
const fs = require('fs');
const os = require('os');
  ...
  try {
    ...
    await check('corrupt, truncated, wrong-shape and over-budget stores remain byte-identical',()=>{
      for(const bad of ['{broken','[','{}','[null]','[{"installId":"x"}]',JSON.stringify([valid]),' '.repeat(4*1024*1024+1)]) {
        ...
        assert.throws(()=>ingest(root,valid),err=>err.code==='E_STORE_CORRUPT');
    ...
    await check('unreadable stores fail closed rather than becoming empty',()=>{
      ...
      try {assert.throws(()=>ingest(root,valid),err=>err.code==='E_STORE_CORRUPT');}
    ...
    await check('failed temporary write or atomic rename preserves prior data and cleans scratch',()=>{
      for(const phase of ['write','rename']) {
        ...
        try {assert.throws(()=>ingest(root,{...valid,toolCalls:3}),err=>err.code==='E_STORE_CORRUPT');}
    ...
    await check('HTTP rejects bad JSON/schema and reports corrupt storage without committing a 200 header',async()=>{
      ...
      try {
        ...
        assert.strictEqual(stats.status,500);assert.strictEqual(JSON.parse(stats.body).code,'E_STORE_CORRUPT');
