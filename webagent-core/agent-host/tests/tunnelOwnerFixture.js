...
const program = process.argv[3] || process.execPath;
...
const child = spawn(program, ['-e', code, stopFile], { stdio: 'ignore', detached: true });
process.on('message', () => {});
setTimeout(() => process.exit(124), 60000).unref();
createRegistry({ baseDirectory: base }).observe(child, 'cloudflare', program).then(result => {
