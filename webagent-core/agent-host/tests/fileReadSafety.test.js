...
// NOTE(merge 01a0c932): this branch raises E_ENCODING where 01a0c932 raised E_INVALID_TEXT.
// Same contract -- invalid UTF-8 is refused and the file is left untouched -- different code
// string. The assertions below track the surviving name; nothing was relaxed.
const fs = require('fs');
const os = require('os');
  ...
  try {
    await check('invalid UTF-8 is rejected, including truncated and overlong sequences', () => {
      for (const bytes of [[0x41,0xff,10], [0x41,0xfe,10], [0xc0,0xaf], [0xe2,0x82], [0xed,0xa0,0x80]]) {
        ...
        assert.throws(() => readBoundedText(path.join(root, 'invalid.txt')), error => error.code === 'E_ENCODING');
