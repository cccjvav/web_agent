...
function harness(options = {}) {
  ...
  // Execute the real preparation helper with this harness's owned spawn and clock. It requires
  // child_process itself, so without this it would escape the sandbox and run a real npm install.
...
(async () => {
  ...
  // --- Backend dependency preparation is not a server: it needs its own deadline, and it must
  // --- not be double-owned by the server-shaped shutdown path. Before this, a stalled registry
  // --- blocked boot indefinitely with no budget at all.
  await test('backend dependency fallback has its own 120s deadline and cannot start late services', async () => {
    ...
    try {
      ...
      assert.equal(h.calls[0].command, 'npm.cmd');
