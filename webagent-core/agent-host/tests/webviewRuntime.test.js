...
(async () => {
  ...
  assert.equal(validWebviewMessage({type:'control',workMode:'chat'},'bridge'),true);
  assert.equal(validWebviewMessage({type:'control',workMode:'code'},'bridge'),false);
  assert.equal(validWebviewMessage({type:'control',revision:'a'.repeat(64),permissions:{read:true,edit:false,execute:false,capture:false}},'bridge'),true);
  assert.equal(validWebviewMessage({type:'control',revision:'a'.repeat(64),permissions:{read:'true',edit:false,execute:false,capture:false}},'bridge'),false);
  let receiver;
  ...
  function runPage(html) {
    ...
    return { context, node, nonce, listeners };
  }
  ...
  assert.equal(node('access-edit').checked,true);
  node('access-edit').checked=false;node('access-edit').onchange();
  assert.equal(node('access-execute').checked,false);
  listeners.message({data:{type:'status',status:{executionControl:ctl}}});
  assert.equal(node('access-edit').checked,false,'poll preserves draft');
  listeners.message({data:{type:'controlSaved'}});
  listeners.message({data:{type:'status',status:{executionControl:ctl}}});
  assert.equal(node('access-edit').checked,true,'successful save permits host readback');
  ...
  context.paintBridgeTasks([{sessionId:'session-a',todos:[{title:'<Remote A>',status:'completed'}]},{sessionId:'session-b',lastMessage:'Progress B',progress:25,todos:[]}]);
  assert.ok(node('task-list').children.some(item=>item.textContent.includes('session-a')));
  assert.ok(node('task-list').children.some(item=>item.textContent.includes('Progress B')));
  assert.equal(node('tasks').style.display,'block');
  context.paintBridgeTasks([]);
  assert.ok(node('task-list').children[0].textContent.includes('set_todos'));
