...
| `/external/*`、`/workflows/*` | GET / POST / DELETE（依实际路由） | external管理以及external/request、workflow预览/提交均要求空query和固定body；未知字段不触网、不保存、不启动/停止进程、不验证工作流或分配审批。合法request只进入待批，不新增远程管理权 |
...
| `/providers/probe`、`/profile/detect` | POST / GET | 均固定query；Provider探测body只接受baseUrl/apiKey且在触网前校验，profile未知query不扫描；合法请求探测模型、环境与技术栈 |
| `/customizations` | GET / PUT | 空query；GET从历史文件只投影固定顶层/嵌套/列表项，PUT严格拒绝空、未知、错类型或超量schema且零写；四文件持久化仍非整体事务 |
