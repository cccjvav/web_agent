...
3. apply_patch. For an existing file only, the host may reuse the last read_files sha256 for that path (only reads observed in this process authorize overwrite; disk cache from an earlier process is not authorization). Prefer passing expectedHash explicitly. If a previously read target disappears, retain its expectedHash and stop to reconcile deletion versus recreation; without expectedHash a missing target follows the creation contract, not an inferred deletion precondition. On HASH_REQUIRED / STALE_FILE, stop this write, re-read current content and reconcile the intended change before preparing a new patch. Never replay a stale patch just by copying currentHash from an error; ask the operator if changes conflict. Unknown mutation outcomes must not be automatically replayed. An existing file needs SEARCH/REPLACE blocks or one unified diff; a body without markers is refused (E_BAD_ARGS), never written as the whole file — use write_file for a deliberate full replacement. Markers are whole lines; an empty REPLACE deletes the matched lines. SEARCH must match once; if it appears more than once pass occurrence (1-based). The host keeps the file line endings (CRLF on Windows).
