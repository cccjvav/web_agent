...
const TOOLS = [
  ...
  tool({
    name: 'apply_patch',
    aliases: [],
    description: 'Atomic SEARCH/REPLACE patch. Prefer expectedHash from the last read_files. For an existing file only, the host may reuse that path’s last read_files sha256. Without expectedHash, a missing target follows the creation contract, not an inferred deletion precondition. HASH_REQUIRED means stop and read the file; do not blindly reuse an error hash or replay a modification. New files / dryRun may omit the hash, but a supplied hash requires the target to still exist. Creation accepts a full body or exactly one empty SEARCH block; existing files accept ONLY SEARCH/REPLACE blocks or a single unified diff — unmarked whole-file text is rejected (use write_file for whole-file replacement). Multiple blocks or nonempty SEARCH require an existing file. STALE_FILE means stop and reconcile, never drop the hash to recreate automatically. SEARCH must match once unless occurrence is set (1-based). Keeps the file CRLF/LF. Code mode only.',
    mode: ['code'],
    inputSchema: {
      type: 'object',
      properties: {
