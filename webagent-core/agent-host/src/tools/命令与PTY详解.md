...
**Windows 退出码传递**：`powershell.exe -Command` 的退出状态取**最后一条语句**的结果，因此guardedCommand必须在尾部恢复用户命令的真实结果，三条合同同时成立（F70重写，windowsExitCodeContract八例在Windows CI真实powershell.exe上通过）：①原生程序的真实退出码保留（F62发现`node failing.js`退出3曾被报成1）；②**纯cmdlet失败仍是失败**——`Get-Item missing.txt`把`$?`置假但从不设置`$LASTEXITCODE`，F62的尾语句`if ($null -ne $LASTEXITCODE) { exit $LASTEXITCODE }`会成为最后一条且`$?`为真的语句，整个脚本退出0，主机把失败命令报成成功（外部复审§5.4-1）；③成功为0。实现：用户命令前先`$global:LASTEXITCODE = $null`，防止前导Add-Type/Attach留下的值冒充用户的；命令之后下一条语句立即`$__wa_ok = $?`；原生码非零优先`exit $LASTEXITCODE`，否则`$__wa_ok`为假则`exit 1`，否则`exit 0`。`$LASTEXITCODE`是**最近一次**原生程序的码：`node fail.js; node ok.js`为0（与sh一致），`node fail.js; Write-Output done`仍为3（cmdlet不重置它，保守地报失败）。`rec.exitCode`/`rec.status`/`rec.ok`都由这个码推导。POSIX 侧 `/bin/bash -c` 本来就直接传递退出码，无需处理。

**Windows 输出编码**（F70，外部复审P1-3）：主机按UTF-8解码子进程输出，但重定向时powershell.exe用OEM代码页写出——自己的cmdlet输出、（本地化的）错误消息，以及它转发的原生程序输出；中文Windows为CP936，所有中文消息到模型手里都成了替换字符（CI的英文runner为CP437，表现为`??`）。guardedCommand第一条语句把`[Console]::OutputEncoding`与`$OutputEncoding`（管道送入原生程序时使用）设为无BOM的UTF-8，再做任何输出；executor三个stdio都是管道，libuv以CREATE_NO_WINDOW创建powershell.exe，它在自己的隐藏控制台里运行，代码页改动不会影响用户终端；没有控制台时赋值抛错被忽略，保持原行为。另外Windows下若用户未设置`PYTHONIOENCODING`，子进程环境补`PYTHONIOENCODING=utf-8`：Windows的Python重定向时用ANSI代码页编码。windowsOutputEncoding在Windows CI覆盖PowerShell stdout/stderr、经PowerShell转发的`cmd /c echo`，装有Python时再测Python；Linux沙箱无法执行，以CI结论为准。仍不覆盖：自行固定输出代码页的程序（例如显式按GBK写出的旧工具）照样会被解成替换字符，那是被调用程序的编码。
