...
async function inspectProcesses(pids) {
  ...
  const script = `$ErrorActionPreference='Stop'; [Console]::OutputEncoding=[Text.UTF8Encoding]::new($false); $rows=@(); foreach($n in @(${unique.join(',')})) { $p=$null; try { $p=[Diagnostics.Process]::GetProcessById($n); $start=$p.StartTime.ToUniversalTime().Ticks.ToString(); $exe=$p.MainModule.FileName; if($p.HasExited -or !$exe) { throw 'unavailable' }; $rows+=@{pid=$n;status='alive';identity=@{pid=$n;start=$start;executable=$exe.ToLowerInvariant()}} } catch [ArgumentException] { $rows+=@{pid=$n;status='absent'} } catch { $rows+=@{pid=$n;status='unknown'} } finally { if($null -ne $p) {$p.Dispose()} } }; ConvertTo-Json -InputObject @($rows) -Depth 4 -Compress`;
  activeWindowsQueries++;
  return new Promise(resolve => {
    const unknown = () => resolve(new Map(unique.map(pid => [pid, { status: 'unknown' }])));
    execFile(shell, ['-NoProfile', '-NonInteractive', '-Command', script], { windowsHide: true, timeout: 8000, maxBuffer: 128 * 1024, encoding: 'utf8' }, (error, stdout) => {
      if (error) return unknown();
      try {
        const rows = JSON.parse(stdout.replace(/^\uFEFF/, ''));
        if (!Array.isArray(rows) || rows.length !== unique.length) return unknown();
        const result = new Map();
        for (const row of rows) {
          if (!unique.includes(row.pid) || result.has(row.pid) || !['alive', 'absent', 'unknown'].includes(row.status)) return unknown();
          if (row.status === 'alive' && (!row.identity || row.identity.pid !== row.pid || !/^\d+$/.test(row.identity.start) || typeof row.identity.executable !== 'string' || !path.isAbsolute(row.identity.executable))) return unknown();
