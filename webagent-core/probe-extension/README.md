...
- `probe_links`、`probe_report` 供已认证客户端读取已共享参考；Bridge工作模式下，远端读取需所有者Read权限；Code 模式 `probe_request` 需所有者四项权限全开，仍只提交一次审批。使用已有 `operation_result` 查询结果。
...
- 只在用户选择后，把参考放入 WebAgent `/ask` 草稿；仍需用户检查并发送；发送前按主机互斥规则结束Bridge工作并明确切换Chat。
- 可选五字段会话挑战独立保留，完整操作已合并到[使用指南](../../使用指南.md)的“可选诊断”小节；实时配对不等于通过挑战，更不证明模型身份。桌面命令顺序是导入最小页面摘要→复制一次性核对请求→外部已认证MCP回传→查询核对结果。丢弃只清扩展当前核对/分析记录，不删除主机TTL记录或已保存历史。
