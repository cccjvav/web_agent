...
| 工作台 REST `POST /chat`、`/bridge/*` | `src/api/routes.js` | [api · routes.js](./shuncode-core/agent-host/src/api/README.md#文件名routesjs) |
...
| 工具 | `…/src/tools/` | 约 25 工具；真正改盘 | [查看详情](./shuncode-core/agent-host/src/tools/README.md) |
...
| 默认工作区 | `workspace/` | 演示计算器 + `.shuncode` | [查看详情](./workspace/README.md) |
...
| 根启动脚本 | 仓库根 `.cmd` / `.sh` | 不是子文件夹 README | [启动脚本说明.md](./启动脚本说明.md) |
...
用户在工作台 CHAT 回车
  ...
  → api/routes.js L147–L169 NDJSON
  → agent/runChat.js
...
云上聊天栏
  ...
  → handleRpc
       ...
       tools/call → callTool(name, args)   ← 不传第三参 mode，Ask 锁对远程不生效
  → 与路径 A 同一套 tools/*
...
**源码事实（避免把指南和实现混为一谈）：** `POST /api/bridge/start`（[routes.js](./shuncode-core/agent-host/src/api/README.md#文件名routesjs)）把 `config.bridgeRunning=true`、写 pairing，**没有**调用 `tunnel.startQuickTunnel`。`mcpOrigin` 仅当 `config.publicTunnelUrl` 已有值才用 `https://*.trycloudflare.com`。隧道模块本身见 [tunnel/README.md](./shuncode-core/agent-host/src/tunnel/README.md)。
