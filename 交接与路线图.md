...
6. 接手后按第4节实际状态选下一项：R1第24组首包已做，续R3设置失败消费链及R2安全依赖，不重做已交付批次；出现新的可复现高风险缺陷优先插队，先记复现与权限边界，再加回归修复。
...
| R3 / 下一项，高 | 待做：先settings.js定制加载/保存失败与旧state提交，再API/工作流长篇 | [API逐项详解](webagent-core/agent-host/src/api/路由逐项详解.md)、operatorQueue/workflows、工具入口与相关测试 | 每路由核对HTTP与业务结果、审批前后复查、deep copy/幂等/取消/unknown；失败不自动重放，不扩大任意命令权限 |
...
本包不提供四文件事务、外部OS写进程隔离或所有辅助字段schema。下一项R3先读`webagent-core/workbench/js/settings.js`的loadCustomizations/saveCustom与bind.js调用：核对HTTP/业务/网络失败是否污染state、是否假报成功、旧state整对象提交是否覆盖其它新值；先复现再修，不自动重试未知结果。R2继续共享路径/读取和安全入口全链审查；不要把本包回归当全部依赖认证。
