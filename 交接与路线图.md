...
本包不提供四文件事务、外部OS写进程隔离或所有辅助字段schema。第25组已修R3定制设置加载/保存与全部saveCustom按钮：失败保留state/草稿且不假成功，只PUT本次字段、保存不重填草稿，页面内busy与10秒取消等待；不是跨标签页CAS，未知写入不自动重试。本地82回归通过，精确CI核对见阶段10。下一项继续`settings.js`的paintProviderTable模型选择、`bind.js`多模型保存及API/工作流长篇的HTTP/业务/失败/审批链，不把上述局部交付当整篇审完。R2继续共享路径/读取和安全入口全链；探测保持暂停。
