...
## 执行流程与边界
工作流顶层权限只授予contents:read；三个job不发布产物、不写仓库。Ubuntu Node18/20/22/24和Windows Node20/22/24安装锁定依赖后先检查文档，再以`npm audit --omit=dev --audit-level=high`把高/严重生产依赖公告设为门禁，随后执行npm test。审计依赖npm公告服务可用，且不覆盖开发依赖、源码逻辑或供应链签名。Windows构建白名单payload，编译输入辅助C#、解析PowerShell，最后编译Inno安装器。Windows步骤成功不是安装、升级或桌面交互成功。
