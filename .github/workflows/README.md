...
## 执行流程与边界
Ubuntu Node18/20/22/24和Windows Node20/22/24安装锁定依赖后执行npm test；文档清单检查是其中的测试，源码新增和hash漂移必须修正后才能通过。Windows构建白名单payload，编译输入辅助C#、解析PowerShell，最后编译Inno安装器。Windows步骤成功不是安装、升级或桌面交互成功。
