...
- **兄弟依赖：** `shuncode-core/scripts/ensure-code-server.js` 往 `code-server-runtime/` 写文件；`run-code-oss.js` 从这里找 `entry.js`。
- **谁调用：** 仅第二种跑法 `run-shuncode-vscode.cmd`。主路径 `run-shuncode.cmd` **不读**本目录。
...
- **每一个 Key：**
  ...
  | Key | 用途 | 取值 |
  |---|---|---|
  | `name` | 包名 | `shuncode-code-server-runtime` |
...
1. 用户第一次跑 `run-shuncode-vscode.cmd`。
...
5. 主路径工作台仍走 `run-shuncode.cmd`，不经过本目录。
