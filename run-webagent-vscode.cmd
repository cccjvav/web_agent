...
echo ===========================================================
echo   Web Agent + 网页 VS Code  ^(Windows CMD^)
echo ===========================================================
echo.
...
if not exist "%WORKSPACE_ROOT%" (
  echo [错误] 工作区不存在：
  echo   %WORKSPACE_ROOT%
  echo 用法: run-webagent-vscode.cmd D:\code\my-app
  pause
  exit /b 1
)

echo 第一次会从 npm 下载 code-server 4.135.0（约 50MB + 依赖）。
echo 请不要和 run-webagent.cmd 同时开（都要用 3000 端口）。
echo 停止: 本窗口 Ctrl+C
echo.

node "%~dp0webagent-core\scripts\run-code-oss.js" "%WORKSPACE_ROOT%"
set "ERR=%ERRORLEVEL%"
if not "%ERR%"=="0" (
  echo.
  echo [错误] 退出代码 %ERR%
