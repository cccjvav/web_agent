...
当前处理目标：`shuncode-core/extension/`

VS Code / code-server 插件源码。侧栏 Chat、Bridge、原生 Chat `@shuncode`。工具实现仍在 agent-host，本目录只做 HTTP 客户端 + webview HTML 字符串。
...
- **每一个 Key：**
  ...
  | Key | 用途 | 取值 |
  |---|---|---|
  | `name` | npm/插件 id 段 | `shuncode-core` |
  | `displayName` | 市场显示名 | `ShunCode AI Agent & Bridge` |
  | `description` | 简介 | 说明连本地 agent-host |
  | `version` | 版本 | `0.6.9`（与 syncExtension 目标目录名一致） |
  | `publisher` | 发布者 | `shuncode` |
  | `engines.vscode` | 最低 VS Code | `^1.90.0` |
  | `categories` | 分类 | Other、Chat |
  | `activationEvents` | 何时激活 | `*` 与 `onChatParticipant:shuncode.agent` |
  | `main` | 入口 | `./extension.js` |
  | `contributes.configuration.properties.shuncode.agentHostUrl` | 可配置 host | 类型 string，默认 `http://127.0.0.1:48271` |
  | `contributes.chatParticipants[0].id` | 参与者 id | `shuncode.agent` |
  | `.fullName` / `.name` | UI 名 | ShunCode / shuncode（`@shuncode`） |
  | `.isSticky` / `.isDefault` | 粘滞、默认 | `true` |
  | `.commands` | slash | `ask` / `plan` / `code` |
  | `viewsContainers.activitybar` | 活动栏容器 | id `shuncode-sidebar`，icon `resources/icon.svg` |
  | `views.shuncode-sidebar` | 两个 webview | `shuncode.chatView`、`shuncode.bridgeView` |
  | `commands` | 命令面板 | `shuncode.openBridge`、`openAgentChat`、`resetSecret` |
...
- **核心类/函数清单：**
  ...
  - **Function `agentHostUrl()`（L6–L9）** — 配置 `shuncode.agentHostUrl` 或 env `SHUNCODE_AGENT_HOST_URL` 或默认 48271，去尾 `/`。
  ...
  - **Function `activate`（L176–L228）** — 注册 ChatView、BridgeView；Chat 参与者；状态栏每 5s GET `/api/status`（运行中 / Agent / 未连接）。命令：打开侧栏；`openAgentChat` 试原生 Chat query `@shuncode `，失败侧栏；resetSecret POST reset-secret。
