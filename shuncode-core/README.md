...
当前处理目标：`shuncode-core/`

这是本仓库 **正在维护的产品代码**。`run-shuncode.cmd` / `run-shuncode.sh` 只启动这里的 `agent-host`。

本层直接文件只有 `start-shuncode.sh` 与本 README。子目录源码的行级说明在各自内部 README，这里不把 `agent-host/src/**` 再抄一遍。
...
- **谁调用：** 根目录 `run-shuncode*.cmd/.sh`、`run-tests.cmd`。
...
| `extension/` | VS Code 插件：侧栏 + `@shuncode` | `extension/README.md` |
...
### 📄 文件名：`start-shuncode.sh`

- **文件职责：** 从 `shuncode-core/` 跳回仓库根，转调根目录 `run-shuncode.sh`。不是 MCP 实现。
- **核心逻辑拆解（文件共 4 行）：**
  - L1：shebang `#!/bin/bash`
  - L2：`set -e`，命令失败即退出
  - L3：`ROOT` = 本脚本所在目录的上一级（仓库根）
  - L4：`exec "$ROOT/run-shuncode.sh"` 替换当前进程，参数不转发（本文件无 `"$@"`）
- **关键变量：** 无配置常量。无函数。
...
1. 用户通常 **不** 先进入本目录，而是双击仓库根 `run-shuncode.cmd` → `agent-host/src/index.js`。
2. 若有人执行本目录 `start-shuncode.sh`：算出仓库根 → `exec run-shuncode.sh` → 与根 bash 入口相同。
3. 网页 VS Code：根 `run-shuncode-vscode.cmd` → `scripts/run-code-oss.js` → 拷 `extension/` 到 `extensions-installed/`，agent-host 跳过 3000。
