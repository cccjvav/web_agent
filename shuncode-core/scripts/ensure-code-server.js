  ...
  if (!entry) {
    ...
    if (!fs.existsSync(pkg)) {
      fs.writeFileSync(
        ...
        JSON.stringify(
          {
            name: 'shuncode-code-server-runtime',
            private: true,
...
function syncExtension() {
  const src = path.join(repoRoot, 'shuncode-core/extension');
  const dest = path.join(repoRoot, 'shuncode-core/extensions-installed/shuncode.shuncode-core-0.6.9');
  ...
  const extRoot = path.join(repoRoot, 'shuncode-core/extensions-installed');
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      [
        {
          identifier: { id: 'shuncode.shuncode-core' },
          version: '0.6.9',
          location: { $mid: 1, path: abs, scheme: 'file' },
          relativeLocation: 'shuncode.shuncode-core-0.6.9'
        }
      ],
      null,
      2
