...
当前处理目标：`shuncode-core/agent-host/`
...
- **兄弟依赖：** 静态页来自 `../workbench/`（`src/index.js` 挂静态目录）。不依赖 `../extension/`（那是 code-server 插件）。不依赖 `../../shuncode-repro/`。
- **谁调用：** 仓库根 `run-shuncode.cmd` / `.sh` 执行 `node src/index.js`；`../scripts/run-code-oss.js` 同样启动并设 `SHUNCODE_SKIP_WORKBENCH=1`；`run-tests.cmd` 跑 `npm test`。
...
- **每一个 Key：**
  ...
  | `scripts.test` | `npm test` | 11 个 `tests/*.test.js` 用 `&&` 串联，顺序：patchEngine → mcpProtocol → workspaceTools → tunnel → httpSmoke → codeServerNotRunnable → skipWorkbench → runChat → chatMode → profile → oauth。任一非 0 即停。 |
  | `keywords` | npm 关键词 | `[]` |
  | `author` | 作者 | `""` |
  | `license` | 许可证 | `ISC` |
  | `description` | 简介 | `""` |
  | `dependencies.cors` | CORS 中间件 | `^2.8.6`（`src/index.js` 使用） |
...
1. `cd shuncode-core/agent-host`（或由根 `.cmd` `cd` 进来）。
