...
当前处理目标：`shuncode-core/agent-host/src/`
...
- **谁调用：** `run-shuncode.cmd` / `.sh` 执行 `node src/index.js`；`scripts/run-code-oss.js` 同样启动并设 `SHUNCODE_SKIP_WORKBENCH=1`。
...
- **关键变量 `config` Key：**
  ...
  | `version` | 展示版本 | `'0.6.9'` |
  | `serverName` / `productName` | MCP serverInfo / 日志 | `ShunCode-AgentHost` / `ShunCode` |
...
- **核心类/函数清单：**
  ...
  - **双服务器（L78–L80, L94–L117）**
    ...
    - L94：`skipWorkbench = SHUNCODE_SKIP_WORKBENCH === '1'`。
