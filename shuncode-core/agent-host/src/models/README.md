...
当前处理目标：`shuncode-core/agent-host/src/models/`

工作区磁盘上的配置、指令、记忆。文件都写在 **被编辑项目** 的 `.shuncode/` 下，不是 Git 仓库根。无独立 `.html`。产出的 json/md 由本目录函数写入。
...
- **文件职责：** 读写 `<工作区>/.shuncode/config.json`。
- **核心类/函数清单：**

  - **Function `dir`（L5–L7）** / **`storePath`（L9–L11）** — `.shuncode` 与其中 `config.json`。
...
- **核心类/函数清单：**
  ...
  - **Function `file`（L6–L8）** — `…/.shuncode/customizations.json`。
...
- **文件职责：** `.shuncode/memory/YYYY-MM-DD.md` 追加备忘。
