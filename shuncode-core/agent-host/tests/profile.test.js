...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'shuncode-profile-'));
...
function main() {
  ...
  assert.ok(fs.existsSync(path.join(tmp, '.shuncode/preference.md')));
  assert.ok(fs.existsSync(path.join(tmp, '.shuncode/tech-stack.md')));
  ...
  fs.mkdirSync(path.join(tmp, '.shuncode/skills/review'), { recursive: true });
  fs.writeFileSync(
    path.join(tmp, '.shuncode/skills/review/SKILL.md'),
    '# Skill: review\n\n用户要求审查时使用。\n'
  );

  const ctx = formatWorkspaceContext(loadCustom(), [{ name: 'review', path: '.shuncode/skills/review', preview: '用户要求审查时使用。' }]);
  ...
  const profile = readResource('shuncode://profile');
