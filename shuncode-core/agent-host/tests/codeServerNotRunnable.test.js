...
const ensure = fs.readFileSync(path.join(repoRoot, 'shuncode-core/scripts/ensure-code-server.js'), 'utf8');
const runner = fs.readFileSync(path.join(repoRoot, 'shuncode-core/scripts/run-code-oss.js'), 'utf8');
...
assert.ok(!/code-server/i.test(mainLaunchers), 'run-shuncode must not start code-server');
assert.ok(/agent-host/.test(mainLaunchers));

assert.ok(fs.existsSync(path.join(repoRoot, 'run-shuncode-vscode.cmd')));
assert.ok(fs.existsSync(path.join(repoRoot, 'shuncode-core/scripts/run-code-oss.js')));
