...
**现行产品不在这里。** 启动脚本不会进入本目录。不要 `cd webagent-repro && npm start`。端口、工具集、密钥存储都和现在的使用指南对不上。
...
当前处理目标：`webagent-repro/`（整棵树：`src/`、`public/`、`tests/`、`workspace_demo/`）。无 Python。
...
- **兄弟依赖：** **没有。** 不 import `webagent-core/`。
...
- **职责：** 原型 npm 清单。
- **Key：**
  - `name`=`webagent-repro`，`version`=`1.0.0`，`main`=`index.js`（实际启动走 scripts，不是根 `index.js`）
...
- `name`=`webagent-demo-project`；`main`=`src/calculator.js`；`scripts.test`=`node tests/calculator.test.js`。无 dependencies。
...
**现行对照（不要在本目录做）：** 产品双端口（工作台 3000 / MCP 48271）、25 工具、OAuth、Chat、密钥可持久化。见 `webagent-core/agent-host/src/README.md`。
