> 当前目标已按用户要求调整为完整参考能力整合，不要求绝对真实。0.2.0已将原型分析内核按白名单复用到VS Code；浏览器联动/历史/自动化等仍在实施。Chat API模型确切验证延期。旧的“产品只接入连接核对”是0.1.x历史状态。
...
自动建档只表示记录了新线索；相似度命中不保证具体模型或版本正确，新模型仍需独立验证。
...
详见[采集层审阅](TRANSPORT_REVIEW.md)。原型仍不随产品自动启动；本次仅隔离测试采集层与UI说明，不运行tools/verify.mjs、CDP或账户/轨迹脚本。原README中的用例数与实测报告是上传时历史记录，不作为当前验证证据。
