...
原recompute每次对同一lastObservation重新runCanaries并直接BUS.evidence.push，既重复累计，也绕过全局上限，而且追加在classify之后，第一次结果看不到该批新增线索。

现在lifecycle.oncePerObservation用WeakSet记住完成观测对象，调用回调前登记，阻止重入；回调异常不自动重放，新的对象才作为新观测。main的appendCanaries每个完成观测仅追加一次，并提前到分类快照之前。BUS.push增加notify参数（默认true），main使用false保持原来的不广播行为，但依然经过历史长度限制。

隔离测试使用中性的计数fixture，不执行原行为探针题库、不向模型提问。验证同对象1000次只写1条、1000个不同对象依然守上限、默认通知不变、静默写入不通知、递归/异常不重放；静态检查main不再绕过BUS.push且先追加再取快照。纯对象去重不解决跨请求证据混用，也不是内容相似度去重。历史存储learnFromObservation是否重复更新、全局run状态竞态仍未解决。

功能差异详见根目录《探针能力对照与迁移边界》：不能将这种去重或采样预算称为保证不降低覆盖率的优化。
