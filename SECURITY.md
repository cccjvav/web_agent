...
目前没有参考截图那样由本机操作者独立勾选Read/Edit/Execute/Capture、并作为所有远程调用权限上限的控制面板。桌面扩展和code-server核心扩展共用主机工具实现，界面模式选择不等于这种权限策略。

| 控制 | 已实现 | 不代表什么 |
|---|---|---|
| 连接认证 | MCP secret或OAuth；本机API/WS限制本机控制面 | 有效凭据不是只读凭据，也不是四类权限范围 |
| Ask/Plan/Code | 共享工具入口按模式拒绝不允许的工具 | 远程请求可自选模式，缺省/未知值回Code；本机Chat切Ask不能锁住远端 |
| 文件访问 | 工作区路径、敏感文件、hash冲突与覆盖保护 | 不是任意命令的文件系统沙箱；hash不是权限开关 |
| 命令/终端 | 常见危险命令拦截；远程禁止交互式PTY；桌面PTY有审批 | 不是所有远程run_command逐条弹窗，也不保证命令不读写文件或截图 |
| 外部工具/工作流 | 特定受控请求等待本机明确批准；stdio启动另行确认 | 不是所有内置文件工具都逐次审批；启动可信程序后仍是OS用户权限 |
| 截图附件 | 路径白名单、PNG/JPEG与6MiB检查 | 当前没有独立Capture禁用开关；可能包含工作区之外的屏幕内容 |

因此目前不能承诺“只允许读取、绝不修改或执行”，也不能承诺“允许任意Execute但绝不Capture”。命令可以跨越文件编辑、读取和截图能力边界，仅隐藏工具名或不显示按钮无法阻止直接调用。

如后续增加分类开关，应由本机所有者设置、在agent-host执行前强制检查（工具列表过滤只能辅助），覆盖别名、工作流和外部工具，并定义关闭权限对已排队/运行任务的影响。若Execute仍可运行任意程序，就必须明确应用层分类不等于OS级隔离。本节是实现范围说明，不表示开关已经施工或获得实机验收。
