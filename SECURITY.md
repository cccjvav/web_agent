...
注册最多80客户端，活跃令牌或授权码保护其注册；满时仅回收超过5分钟的无活动注册，否则503，不撤销正常连接来腾位置。回调最多16个、每个2048字符，只支持HTTPS或HTTP回环，精确匹配；不支持自定义URI scheme。公开授权GET不再生成/更新码，须从本机工作台生成。授权请求另有IP限流，但全局五次错误锁定仍可能被恶意请求耗尽，不声称消除配对拒绝服务。
