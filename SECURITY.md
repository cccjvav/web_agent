...
这改变的是工具路径，用户手动终端仍按自己的配置运行；使用LFS/转换驱动的项目可能看到不同差异或触及输出预算。默认、目录和staged diff先枚举NUL元数据，检查每个路径与Git识别的重命名两侧，再用显式白名单/--no-renames输出；拒绝跨工作区来源，省略数及truncated说明不完整。子目录工作区的status也限定范围。普通合法文件内未命名为敏感的秘密不自动识别，不能把输出当通用脱敏导出或内容来源证明。Git二进制、系统环境和稳定配置属于本机信任边界，不能隔离同OS用户恶意替换程序/并发修改配置。每次内部git包装约8秒剩余预算（一个工具会调用多次）、512KiB子进程输出上限及响应行数限制不证明所有任意仓库都能完成。
