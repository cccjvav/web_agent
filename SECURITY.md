...
权限保存只走本机控制面并核对工作区、hostInstanceId和策略revision。正在执行或有后台程序时拒绝切换/撤权，不隐式强杀。等待审批允许撤权，批准时复查；等待审批仍阻止跨模式。子进程未close、stdio未closed、PTY取消未获所有者确认仍阻止切换。详细操作与验证范围见[Bridge权限与工作模式](Bridge权限与工作模式.md)。
