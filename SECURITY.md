...
连接核对的页面摘要是自报信息，挑战回传只证明已认证MCP会话持有该挑战，不鉴定后台模型/人类账户/页面真实性，也不新增权限。可选用户脚本只在主动点击后输出最小上下文，无网络钩子/令牌访问；完整研究原型不随产品启用。原型recon原始抓包已移出后续Git跟踪，但历史并未清除；仍有效凭据需在来源平台按其能力撤销，不得将原始dump提交至诊断接口。
