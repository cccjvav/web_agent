...
> 以下92/123/127等计数属于历史增量记录，不是当前覆盖率。当前问题、已修范围与未验收项见[当前维护者交叉验证台账](./review/CURRENT_AUDIT.md)。链接有效不等于文档所述功能已实现。

> 2026-09-12起采用[统一维护规范](manager/docs/documentation.md)与自动清单；实时结构检查运行 `node docs-site/check-docs.js`，符号索引见[自动索引](docs-site/source-index.md)。旧统计保留为历史，不再手工递增。结构登记不是语义准确率。

对照第一～三阶段生成的子文件夹 `README.md` 与根目录 [`总览.md`](./总览.md) 做覆盖率、链接、格式检查。本报告不是产品使用指南。

审查日期：2026-09-03（计数口径）；分支：`arena/01a05d84-web-agent`。

过程文档在 [review/](./review/)（REPORT v1–v6、PROMPT、Windows 真机清单）**不计入**下面的代码文件覆盖率。用户操作指南在仓库根 Markdown，不全部嵌进 docs-site（见 [docs-site/README.md](./docs-site/README.md) 收录范围）。
...
| 根目录补充说明书（不是文件夹 README） | [`启动脚本说明.md`](./启动脚本说明.md)、[`总览.md`](./总览.md)、[`架构导读.md`](./架构导读.md)、[`CONTRIBUTING.md`](./CONTRIBUTING.md)、本文件 |
| 被至少一份说明书点名的代码/配置文件 | **127** |
| 按扩展名 | `.js` 98、`.json` 8、`.cmd` 5、`.sh` 4、`.svg` 3、`.html` 2、`.css` 2、`.yaml` 2、`.gitignore` 1、`.gitattributes` 1 |
| [`总览.md`](./总览.md) 内 Markdown 链接 | **72**，相对路径全部能解析到文件（`./` 正确，无误用 `../`） |
