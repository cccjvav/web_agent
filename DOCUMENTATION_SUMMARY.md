...
| 被至少一份说明书点名的代码/配置文件 | **108** |
| 按扩展名 | `.js` 83、`.json` 8、`.cmd` 4、`.sh` 3、`.svg` 3、`.html` 2、`.css` 2、`.yaml` 1、`.gitignore` 1、`.gitattributes` 1 |
...
现：**108 / 108**。工作台把原来的单文件 `app.js` IIFE 拆成 `js/state.js`、`js/dom.js`、`js/tabs.js`、`js/chat.js`、`js/bridge.js`、`js/settings.js`、`js/bind.js`（+7），均写入 [`workbench/README.md`](./webagent-core/workbench/README.md)。P0 后又加 `src/utils/localControl.js` 与 `tests/localControl.test.js`。P1 加 `tests/sandbox.test.js`（真实路径沙箱）、`src/utils/corsAllow.js` 与 `tests/corsAllow.test.js`（MCP CORS 白名单）。仓库级约定是 Skill [`docs-sync`](./workspace/.webagent/skills/docs-sync/SKILL.md)（`load_skill` 名 `docs-sync`），不是仓库根 `文档约定.md`。该 Skill 还要求：动到「为什么这样装」时按四层写法改根目录 [`架构导读.md`](./架构导读.md)（产品文，不计入上面的代码文件数）。
